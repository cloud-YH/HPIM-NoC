...
class behavior_mapping(tile):
    def __init__(self, NetStruct, SimConfig_path):
        ...
        #get mix_mode
        self.bm_mix_mode = int(bm_config.get('Device level','Device_mix_mode'))
        tile.__init__(self, SimConfig_path,inter_mix_type='none')
		#mix_mode=2 is about to add :among the tile
        #linqiushi above
    ...
    def config_behavior_mapping(self):
        #use flag to change NVM and SRAM alternately
        flag=0
                ...
                while channel_width_2bsplit > 0:
                    ...
                    while weight_precision_2bsplit > 0:
                        ...
                        if current_PE_num == self.tile_PE_total_num or \
                            ...
                            # print("yes")
                            #linqiushi modified
                            if self.bm_mix_mode==2:
                                if flag==0:
                                    __temp_tile = tile(self.SimConfig_path,inter_mix_type='NVM')
                                    flag=1
                                elif flag==1:
                                    __temp_tile = tile(self.SimConfig_path,inter_mix_type='SRAM')
                                    flag=0
                            elif self.bm_mix_mode==1:
                                __temp_tile = tile(self.SimConfig_path,mix_type='none')
                            elif self.bm_mix_mode==3:
                                __temp_tile = tile(self.SimConfig_path,mix_type='none')
                            else:
