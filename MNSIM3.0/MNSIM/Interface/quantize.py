...
class MyModel_MM(nn.Module):
    def __init__(self, in_features, layer_config):
        super(MyModel_MM, self).__init__()
        self.layer_config = layer_config
        self.weight = nn.Parameter(torch.randn(self.layer_config['features'],in_features))
        

    def forward(self, x):
        output = torch.matmul(x,self.weight.transpose(0,1))
...
class ViewLayer(nn.Module):
    ...
    def forward(self, x):
        x=x.view(x.size(0), -1)
        return x
class ViewMM(nn.Module):
    def __init__(self):
        super(ViewMM, self).__init__()
    def forward(self, x):
...
class ConcatLayer(nn.Module):
    ...
    def forward(self, x):
        return torch.cat([xi for xi in x], len(x[0].shape)-1)
...
class StraightLayer(nn.Module):
    def __init__(self, hardware_config, layer_config, quantize_config):
        ...
        elif self.layer_config['type'] == 'viewMM':
            self.layer= ViewMM()
    ...
    def extra_repr(self):
        return str(self.hardware_config) + ' ' + str(self.layer_config) + ' ' + str(self.quantize_config)
StraightLayerStr = ['pooling', 'relu','Swish','Sigmoid','Softmax','view', 'viewMM','concat', 'bn', 'dropout', 'element_sum','element_multiply']
