    ...
    def get_structure(self):
        ...
        for i, layer in enumerate(self.layer_list):
            ...
            if len(input_index) == 1:
                ...
                #print("weishenm",tensor_list[input_index[0] + i + 1].shape,tensor_list[input_index[1] + i + 1].shape)
                tensor_list.append(
...
def get_net(hardware_config = None, cate = 'lenet', num_classes = 10):
    ...
    elif cate.startswith('Attention_example'):
        # a simple example of Attention(single-head)
        
        #WQ,WK,WV
        # layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,300],'features':300})
        # layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,300],'features':300,'input_index': [-2]})
        # layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,300],'features':300,'input_index': [-3]})
        # layer_config_list.append({'type':'MM','input1_size':[300,300],'input2_size':[300,300],'features':300,'input_index': [-1]})
        # layer_config_list.append({'type': 'Softmax'})
        # layer_config_list.append({'type':'MM','input1_size':[300,300],'input2_size':[300,300],'features':300,'input_index': [-1]})
        # layer_config_list.append({'type': 'view'})
        # layer_config_list.append({'type':'fc','in_features':90000,'out_features':10})
        
        #WQ,WK,WV
        #encoder
        #into multi-head(4 head)
        #300:token
        #4096: length of embedding vector
        #WQ
        
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024})
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-2]})
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-3]})
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-4]})
        #WK
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-5]})
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-6]})
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-7]})
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-8]})
        #WV
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-9]})
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-10]})
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-11]})
        layer_config_list.append({'type':'MM','input1_size':[300,4096],'input2_size':[4096,1024],'features':1024,'input_index': [-12]})
        #QK^T(into 4 groups)
        layer_config_list.append({'type':'MM','input1_size':[300,1024],'input2_size':[1024,300],'features':300,'input_index': [-8]})
        layer_config_list.append({'type':'MM','input1_size':[300,1024],'input2_size':[1024,300],'features':300,'input_index': [-8]})
        layer_config_list.append({'type':'MM','input1_size':[300,1024],'input2_size':[1024,300],'features':300,'input_index': [-8]})
        layer_config_list.append({'type':'MM','input1_size':[300,1024],'input2_size':[1024,300],'features':300,'input_index': [-8]})
        # softmax(into 4 groups)
        layer_config_list.append({'type': 'Softmax','input_index': [-4]})
        layer_config_list.append({'type': 'Softmax','input_index': [-4]})
        layer_config_list.append({'type': 'Softmax','input_index': [-4]})
        layer_config_list.append({'type': 'Softmax','input_index': [-4]})
        # softmax(QK^T)*V(into 4 groups)
        layer_config_list.append({'type':'MM','input1_size':[300,300],'input2_size':[300,1024],'features':10000,'input_index': [-5]})
        layer_config_list.append({'type':'MM','input1_size':[300,300],'input2_size':[300,1024],'features':1024,'input_index': [-6]})
        layer_config_list.append({'type':'MM','input1_size':[300,300],'input2_size':[300,1024],'features':1024,'input_index': [-7]})
        layer_config_list.append({'type':'MM','input1_size':[300,300],'input2_size':[300,1024],'features':1024,'input_index': [-8]})
        # concat(never used before)
        layer_config_list.append({'type':'concat','input_index': [-1,-2]})
        layer_config_list.append({'type':'concat','input_index': [-1,-3]})
        layer_config_list.append({'type':'concat','input_index': [-1,-4]})
        # shape:[300,300]
        # the linear after multi-head
        layer_config_list.append({'type':'MM','input1_size':[300,300],'input2_size':[300,4096],'features':4096})
        # feed forward layer
        # keep the shape
