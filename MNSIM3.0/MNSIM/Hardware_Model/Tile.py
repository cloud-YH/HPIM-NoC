...
class tile(ProcessElement):
	def __init__(self, SimConfig_path,device_type='none',xbar_size=[0,0],PE_num=0,mix_mode=1,easy=0):
		# layer_num is a list with the size of 1xPE_num
		if easy==0:
			if mix_mode==2 or mix_mode==4:
				ProcessElement.__init__(self,SimConfig_path,device_type=device_type,xbar_size=xbar_size,ADC_num=math.ceil(xbar_size[0]/8),DAC_num=math.ceil((xbar_size[0]/8)))
			else:
				ProcessElement.__init__(self, SimConfig_path,device_type=device_type,xbar_size=xbar_size)
			...
			if PE_num!=0:
				self.tile_PE_num[0] = PE_num
				self.tile_PE_num[1] = PE_num
			...
			#3:mix among tile
			#add the mix 
			#mix_mode=2 is about to add
			count=0
			if mix_mode==2 or mix_mode==4 and xbar_size[0]!=0:
			#ADC_num,DAC_num:xbar_size/16
				__PE = ProcessElement(SimConfig_path,device_type=device_type,xbar_size=xbar_size,ADC_num=math.ceil(xbar_size[0]/8),DAC_num=math.ceil((xbar_size[0]/8)))
			else:
				__PE = ProcessElement(SimConfig_path,device_type=device_type,xbar_size=xbar_size)
			for i in range(self.tile_PE_num[0]):
				...
				for j in range(self.tile_PE_num[1]):
					...
					# 	#3:inside the tile
					# 	count+=1
					# 	if count>self.tile_PE_num[0]*self.tile_PE_num[1]/2:
					# 		__PE = ProcessElement(SimConfig_path,device_type=device_type,xbar_size=xbar_size)
					# 	else:
					# 		__PE = ProcessElement(SimConfig_path,device_type=device_type,xbar_size=xbar_size)
					# elif mix_mode ==1:
					...
					# 	__PE = ProcessElement(SimConfig_path,device_type=device_type,xbar_size=xbar_size)
					# else :
					# 	assert 0, f'mix mode error: should in range {1,2,3}'
					#TCG->tile->PE
	...
	def calculate_intra_PE_connection(self):
		...
		#print('tett',self.tile_PE_list[0][0].PE_ADC_num)
	...
	def calculate_tile_area(self, SimConfig_path=None, default_inbuf_size = 16, default_outbuf_size = 4,mix_mode=1,ADC_num_mix=0,DAC_num_mix=0):
		# unit: um^2
		self.tile_area = 0
		self.tile_xbar_area = 0
		self.tile_ADC_area = 0
		...
		for i in range(self.tile_PE_num[0]):
			for j in range(self.tile_PE_num[1]):
				self.tile_PE_list[i][j].calculate_PE_area(SimConfig_path=SimConfig_path, default_inbuf_size = default_inbuf_size,mix_mode=mix_mode,
	...
	def calculate_tile_area_part(self, SimConfig_path=None, mix_mode=1,ADC_num_mix=0,DAC_num_mix=0):
		# unit: um^2
		self.tile_area = 0
		self.tile_xbar_area = 0
		self.tile_ADC_area = 0
		...
		for i in range(self.tile_PE_num[0]):
			for j in range(self.tile_PE_num[1]):
				self.tile_PE_list[i][j].calculate_PE_area_part(SimConfig_path=SimConfig_path,mix_mode=mix_mode,
                                              ADC_num_mix=ADC_num_mix,DAC_num_mix=DAC_num_mix)
				self.tile_xbar_area += self.tile_PE_list[i][j].PE_xbar_area
				self.tile_ADC_area += self.tile_PE_list[i][j].PE_ADC_area
				self.tile_DAC_area += self.tile_PE_list[i][j].PE_DAC_area
		...
		self.tile_digital_area = self.tile_input_demux_area + self.tile_output_mux_area + self.tile_adder_area \
								 + self.tile_shiftreg_area + self.tile_jointmodule_area + self.tile_iReg_area + self.tile_oReg_area
		self.tile_pooling_area = self.tile_pooling.Pooling_area
		self.tile_area = self.tile_xbar_area + self.tile_ADC_area + self.tile_DAC_area + self.tile_digital_area + self.tile_pooling_area


	def calculate_tile_read_power_fast(self, max_column=0, max_row=0, max_PE=0, max_group=0, layer_type=None,
									   SimConfig_path=None, default_inbuf_size = 16, default_outbuf_size = 4,mix_mode=1,ADC_num_mix=0,DAC_num_mix=0):
	...
	def calculate_tile_write_power_fast(self,max_column=0,max_row=0,max_group=0,max_PE=0):
		self.tile_write_power=0
		self.tile_xbar_write_power=0
		self.calculate_PE_write_power_fast(max_column=max_column,max_row=max_row,max_group=max_group)
		self.tile_xbar_write_power=max_PE*self.PE_xbar_write_power
		self.tile_write_power=self.tile_xbar_write_power
