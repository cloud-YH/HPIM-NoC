...
# 创建一个自定义的HSV颜色映射，从蓝色（低温）到红色（高温）
def custom_hsv_cmap(num_colors=256):
    # 创建一个线性变化的色调数组，从蓝色（0.667）到红色（0）
    hue = np.linspace(0.667, 0, num_colors)
    saturation = np.ones(num_colors)  # 饱和度全为1
    value = np.ones(num_colors)       # 亮度全为1
...
cmap = custom_hsv_cmap()
im = axs.imshow(temps, cmap=cmap , extent=(0, total_width, 0, total_length))

# if min_temp is None and max_temp is None:
#   im.set_clim(np.min(temps), np.max(temps))
# else:
#   im.set_clim(min_temp, max_temp)
im.set_clim(318.15, 323.15)
...
axs.set_xticklabels([f"{n*(10**3):.2f}" for n in np.linspace(0, total_width, 5)])
axs.set_xlabel("Horizontal Position (mm)")

axs.set_yticks([n for n in np.linspace(0, total_length, 5)])
axs.set_yticklabels([f"{n*(10**3):.2f}" for n in np.linspace(0, total_length, 5)])
